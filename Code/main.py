...
if __name__ == "__main__":
    ...
    for filename in os.listdir(Dir):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            img_read += 1
            img = imageio.imread(Dir + '/' + filename, pilmode='RGB')
            img = np.array(Image.fromarray(img).resize((224, 224))).astype(np.float32)
            img[:, :, 0] -= 123.68
            img[:, :, 1] -= 116.779
            img[:, :, 2] -= 103.939
            ...
            print('Total image read: ', img_read, 'Average Process Time: ', total_time/img_read, 's/image, Average image/s: ', img_read/total_time)
